...
def test_py_u_01_valid_json(mocker):
    mocker.patch("subprocess.run", return_value=MagicMock(returncode=0, stdout='{"hostname": "TestBox"}'))
...
def test_py_u_03_malformed_json(mocker):
    mocker.patch("subprocess.run", return_value=MagicMock(returncode=0, stdout="hostname missing brackets entry"))
...
def test_py_u_07_validation_reject_missing_host(mocker):
    mocker.patch("subprocess.run", return_value=MagicMock(returncode=0, stdout='{"os_version": "Win11"}'))
