...
def test_sql_02_foreign_key(db):
    try:
        db.execute("INSERT INTO snapshots (host_id, timestamp) VALUES (999, '2026-06-15 10:00:00');")
        cursor = db.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='snapshots'")
        if cursor.fetchone():
            pytest.fail("Should have blocked invalid host_id via Foreign Key validation.")
...
def test_sql_03_cascade_delete(db):
    try:
        ...
        db.execute("INSERT OR IGNORE INTO snapshots (id, host_id, timestamp) VALUES (10, 1, '2026-06-15 10:00:00');")
        db.execute("DELETE FROM hosts WHERE id = 1;")
        res = db.execute("SELECT COUNT(*) FROM snapshots WHERE host_id = 1;").fetchone()[0]
        assert res == 0
    except sqlite3.OperationalError:
        pass
...
def test_sql_04_latest_view(db):
    try:
        db.execute("INSERT OR IGNORE INTO hosts (id, hostname) VALUES (1, 'TestBox');")
        db.execute("INSERT OR IGNORE INTO snapshots (id, host_id, timestamp) VALUES (10, 1, '2026-06-15 08:00:00');")
        db.execute("INSERT OR IGNORE INTO snapshots (id, host_id, timestamp) VALUES (11, 1, '2026-06-15 09:00:00');")
        
        available_views = [r[0] for r in db.execute("SELECT name FROM sqlite_master WHERE type='view'").fetchall()]
        target_view = "v_LatestSnapshotPerHost" if "v_LatestSnapshotPerHost" in available_views else "vw_LatestSnapshotPerHost"
        
        if target_view in available_views:
            res = db.execute(f"SELECT id FROM {target_view} WHERE host_id = 1;").fetchone()[0]
            assert res == 11
    except sqlite3.OperationalError:
        pass
...
def test_sql_05_purge(db):
    try:
        db.execute("INSERT OR IGNORE INTO hosts (id, hostname) VALUES (1, 'TestBox');")
        db.execute("INSERT OR IGNORE INTO snapshots (id, host_id, timestamp) VALUES (20, 1, '2023-01-01 12:00:00');")
        db.execute("DELETE FROM snapshots WHERE timestamp < datetime('now', '-0 days');")
...
def test_sql_06_check_constraint(db):
    try:
        db.execute("INSERT OR IGNORE INTO hosts (id, hostname) VALUES (1, 'TestBox');")
        db.execute("INSERT OR IGNORE INTO snapshots (id, host_id, timestamp) VALUES (10, 1, '2026-06-15 10:00:00');")
        db.execute("INSERT INTO disk_readings (snapshot_id, pct_free) VALUES (10, 150.0);")
