...
def test_py_i_01_tables_exist(db_session):
    cursor = db_session
    cursor.execute(
        "SELECT name FROM sqlite_master WHERE type='table' AND name IN ('hosts', 'snapshots')"
    )
...
def test_py_i_02_collect_inserts_rows(db_session):
    ...
    cursor.execute(
        "INSERT INTO snapshots (host_id, timestamp) VALUES (1, '2026-06-15 12:00:00')"
    )
    cursor.execute("INSERT INTO disk_readings (snapshot_id, pct_free) VALUES (1, 45.5)")
    cursor.execute(
        "INSERT INTO process_readings (snapshot_id, process_name) VALUES (1, 'pwsh')"
    )
...
def test_py_i_06_check_constraint_blocks_negative(db_session):
    ...
    with pytest.raises(sqlite3.IntegrityError):
        cursor.execute(
            "INSERT INTO disk_readings (snapshot_id, pct_free) VALUES (1, -5.0)"
        )
